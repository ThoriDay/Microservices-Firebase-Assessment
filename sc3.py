...
app = Flask(__name__)
DATABASE = 'cells.db'
FIREBASE_DB_URL = "https://ecm3408-75ba0-default-rtdb.europe-west1.firebasedatabase.app/"
...
def create_cell(cell_id):
    ...
    if not data or 'id' not in data or 'formula' not in data or data['id'] != cell_id:
        return make_response(jsonify({"error": "Missing id or formula, or id mismatch"}), 400)
